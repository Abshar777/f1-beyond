  ...
  return (
    <section id="markets" className="category2 pb-px">
      <div className="mx-auto w-full max-w-[1320px] px-4 sm:px-6 lg:px-8">
        <div className="category2__wrap border-b border-border py-20 min-[1400px]:py-[90px] min-[1920px]:py-[130px]">
          <div className="flex flex-col gap-12 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="w-full lg:w-7/12 xl:pl-4 xl:pr-1">
              <Reveal
                stagger
                className="category2__grid-row grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3"
              >
                {CATEGORIES.map((category) => (
                  <a
                    key={category.name}
                    href="#programmes"
                    className="category2__card group flex h-full items-center gap-2.5 rounded-[10px] border border-border bg-white p-[9px] transition-all duration-300 hover:-translate-y-1 hover:border-secondary/45 hover:shadow-[0_14px_30px_-18px_rgba(212,175,55,0.75)]"
                  >
                    {/* gold on hover — this was bg-primary, which now resolves
                        to zinc-900 and turned the whole tile black */}
                    <div className="category2__card-icon flex h-[60px] w-[60px] shrink-0 items-center justify-center rounded-[6px] bg-bg text-primary transition-colors duration-300 group-hover:bg-secondary group-hover:text-primary [&>svg]:h-[30px] [&>svg]:w-[30px]">
                      <category.Icon strokeWidth={1.6} aria-hidden />
                    </div>
                    <div className="category2__card-content flex flex-col">
                      <div className="category2__card-name mb-2 font-mona text-base font-medium capitalize leading-[120%] text-primary transition-colors duration-300 group-hover:text-secondary">
                        {category.name}
                      </div>
                      <span className="category2__card-learners font-mona text-sm capitalize leading-[100%] text-text">
